...
def connect_to_db():
    try:
        connection = mysql.connector.connect(
            host='175.125.92.196',       # MySQL 서버 주소
...
def insert_data_from_xls(file_path, table_name):
    ...
    for index, row in df.iterrows():
        # 테이블 컬럼에 맞게 쿼리 작성 (예시)
        sql = f"INSERT INTO bookcase1 (title, writer, publisher) VALUES (%s, %s, %s)"
        values = (row['제목'], row['작가'], row['출판사'])
...
file_path = '/home/liam/workspace/virtual/10_project/codes/each_func/01.switch_data/bookcase1_3.xls'
