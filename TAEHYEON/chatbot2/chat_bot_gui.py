...
def search_book(connection, search_type, search_term):
    try:
        ...
        # 검색 유형에 따라 쿼리 변경
        if search_type == "제목":
            query = "SELECT * FROM bookcase1 WHERE title LIKE %s"
        elif search_type == "저자":
            query = "SELECT * FROM bookcase1 WHERE writer LIKE %s"
        elif search_type == "출판사":
            query = "SELECT * FROM bookcase1 WHERE publisher LIKE %s"
        elif search_type == "장르":
            query = "SELECT * FROM bookcase1 WHERE bookcaseSubject LIKE %s"
...
# 검색 버튼 클릭 시 실행되는 함수
def on_search():
    search_type = search_type_var.get()  # 사용자가 선택한 검색 유형 (제목, 저자, 출판사, 장르)
    ...
    if connection:
        ...
        # 검색 결과 처리
        if results:
            result_text = "\n".join([f"제목: {row['title']}, 저자: {row['writer']}, 출판사: {row['publisher']}, 장르: {row['bookcaseSubject']}" for row in results])
...
search_type_combobox = ttk.Combobox(root, textvariable=search_type_var, values=["제목", "저자", "출판사", "장르"], state="readonly")
