...
# 책 정보 검색
def search_book(connection, search_term):
    try:
        cursor = connection.cursor(dictionary=True)
        
        query = """
            SELECT * FROM bookcase2 
            WHERE title LIKE %s OR writer LIKE %s OR bookcaseSubject LIKE %s OR publishingDate LIKE %s
        """
...
def on_search(event=None):
    ...
    # 책 정보 검색
    book_results = search_book(connection, search_text)
    
    if book_results:
        book_text = "\n".join([f"제목: {row['title']}, 작가: {row['writer']}, 장르: {row['bookcaseSubject']}, 출간일: {row['publishingDate']}" for row in book_results])
        response += f"\n\n책 정보:\n{book_text}"
